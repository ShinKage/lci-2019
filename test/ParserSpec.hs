...
parserSpec =
  describe "parsing" $ do
    ...
    it "parse boolean literals" $ do
      parse (atom <* eof) "" "true" `shouldParse` UBoolE True
      parse (atom <* eof) "" "false" `shouldParse` UBoolE False
    it "parse allowed identifiers" $ do
      parse (atom <* eof) "" "abc123_" `shouldParse` UVar "abc123_"
      parse (atom <* eof) "" "abc_123" `shouldParse` UVar "abc_123"
      parse (atom <* eof) "" `shouldFailOn` "_a"
      parse (atom <* eof) "" `shouldFailOn` "2a"
    it "fails for reserved keywords" $ forM_ reserved $
      \k -> parse (identifier <* eof) "" `shouldFailOn` pack k
    it "parse unit literal" $
      parse (atom <* eof) "" "()" `shouldParse` UUnitE
    it "parse conditional branches" $ do
      parse (atom <* eof) "" "if true then 1 else 2" `shouldParse` UCond (UBoolE True) (UIntE 1) (UIntE 2)
      parse (atom <* eof) "" `shouldFailOn` "if true then 1"
    it "parse pairs of expressions" $ do
      parse (atom <* eof) "" "{1, 2}" `shouldParse` UPair (UIntE 1) (UIntE 2)
      parse (atom <* eof) "" "{42, true}" `shouldParse` UPair (UIntE 42) (UBoolE True)
      parse (atom <* eof) "" "{{1, 2}, 3}" `shouldParse` UPair (UPair (UIntE 1) (UIntE 2)) (UIntE 3)
    ...
    it "should parse fixpoints" $
      parse (fix <* eof) "" "fix 1" `shouldParse` UFix (UIntE 1)
